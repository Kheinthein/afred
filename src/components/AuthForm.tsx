...
export function AuthForm({ mode }: AuthFormProps): JSX.Element {
  ...
  const onSubmit = async (values: AuthFormValues): Promise<void> => {
    setError(null);
    try {
      const payload: LoginPayload | RegisterPayload = {
        email: values.email,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700" htmlFor={`${mode}-email`}>
          Email
        </label>
        <input
          id={`${mode}-email`}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700" htmlFor={`${mode}-password`}>
          Mot de passe
        </label>
        <input
          id={`${mode}-password`}
